...
public:
  int nearestExit(vector<vector<char>> &maze, vector<int> &entrance) {
    ...
    auto is_exit = [&](int i, int j) {
      ...
      return (maze[i][j] == '.') and
             (i == 0 or i == max_rows - 1 or j == 0 or j == max_cols - 1);
    };
    auto is_valid = [&](int i, int j) {
      return (i >= 0 and i < max_rows and j >= 0 and j < max_cols) and
             (maze[i][j] == '.');
    };
    ...
    while (!q.empty()) {
      ...
      while (level_size--) {
        ...
        for (auto dir : dirs) {
          pair<int, int> next_pos = {u.first + dir.first,
                                     u.second + dir.second};
          if (is_valid(next_pos.first, next_pos.second) and
              !visit[next_pos.first][next_pos.second]) {
